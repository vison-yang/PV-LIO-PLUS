...
{
    template <>
    struct hash<voxel_map_plus_ns::VOXEL_LOC>
    {
        int64_t operator()(const voxel_map_plus_ns::VOXEL_LOC &s) const
        {
            using std::hash;
            using std::size_t;
...
namespace voxel_map_plus_ns
{
    ...
    public:
        ...
        void InitPlane(const std::vector<pointWithCov> &points, const PlanePtr &plane, UnionFindNode *node) const
        {
            ...
            {
                now_center += points[last_size + i].point_world;
                now_EX2 += points[last_size + i].point_world * points[last_size + i].point_world.transpose();
                plane->xx += points[last_size + i].point_world[0] * points[last_size + i].point_world[0];
                plane->yy += points[last_size + i].point_world[1] * points[last_size + i].point_world[1];
                plane->zz += points[last_size + i].point_world[2] * points[last_size + i].point_world[2];
                plane->xy += points[last_size + i].point_world[0] * points[last_size + i].point_world[1];
                plane->xz += points[last_size + i].point_world[0] * points[last_size + i].point_world[2];
                plane->yz += points[last_size + i].point_world[1] * points[last_size + i].point_world[2];
                plane->x += points[last_size + i].point_world[0];
                plane->y += points[last_size + i].point_world[1];
                plane->z += points[last_size + i].point_world[2];
            }
            ...
            if (evalsReal(evalsMin) < planer_threshold)
            {
                ...
                if (fabs(evecMin[0]) >= fabs(evecMin[1]) && fabs(evecMin[0]) >= fabs(evecMin[2]))
                {
                    ...
                    for (auto pv : points)
                    {
                        double xi = pv.point_world[0];
                        double yi = pv.point_world[1];
                        double zi = pv.point_world[2];
                        ddetA_dpw << 0.0,
                        ...
                        dAstarE_dpw << yi * z * z - n * yi * zz + n * zi * yz - zi * y * z + zz * y - yz * z,
                            ...
                            2 * xy * z - 2 * n * zi * xy + n * (xi * yz + yi * xz) - y * (xz + xi * z) + 2 * zi * x * y - x * (yz + yi * z),
                            n * yi * yz - yi * y * z + zi * y * y - n * zi * yy + yy * z - yz * y,
                            n * (xi * yz + zi * xy) - z * (xy + xi * y) + 2 * xz * y - 2 * n * yi * xz + 2 * yi * x * z - x * (yz + zi * y),
                            n * yi * xy - xy * y + xi * y * y - n * xi * yy + x * yy - yi * x * y,
                            yi * zz * y - yi * yz * z + zi * yy * z - zi * yz * y + yz * yz - yy * zz,
                            zz * (xy + xi * y) - z * (xi * yz + zi * xy) + 2 * yi * xz * z - xz * (yz + zi * y) + 2 * zi * x * yz - 2 * yi * x * zz,
                            2 * zi * xy * y - xy * (yz + yi * z) + yy * (xz + xi * z) - y * (xi * yz + yi * xz) + 2 * yi * x * yz - 2 * zi * x * yy;
                        J_pw = A_star * E * (-1.0 * ddetA_dpw / detA / detA).transpose() + dAstarE_dpw / detA;
                        plane->plane_cov += J_pw * pv.cov_world * J_pw.transpose();
                    }
                }
                else if (fabs(evecMin[1]) >= fabs(evecMin[0]) && fabs(evecMin[1]) >= fabs(evecMin[2]))
                {
                    ...
                    for (auto pv : points)
                    {
                        ...
                        dAstarE_dpw << yi * z * z - n * yi * zz + n * zi * yz - yz * z + y * zz - zi * y * z,
                            ...
                            2 * xy * z - 2 * n * zi * xy + n * (yi * xz + xi * yz) - x * (yz + yi * z) + 2 * zi * y * x - y * (xz + xi * z),
                            n * (yi * xz + zi * xy) - z * (xy + yi * x) + 2 * yz * x - 2 * n * xi * yz + 2 * xi * y * z - y * (xz + zi * x),
                            n * xi * xz - xi * x * z + zi * x * x - n * zi * xx + xx * z - xz * x,
                            n * xi * xy - xy * x + yi * x * x - n * yi * xx + y * xx - xi * y * x,
                            zz * (yi * x + xy) - z * (yi * xz + zi * xy) + 2 * xi * yz * z - yz * (xz + zi * x) + 2 * zi * y * xz - 2 * xi * y * zz,
                            xi * zz * x - xi * xz * z + zi * xx * z - zi * xz * x + xz * xz - xx * zz,
                            2 * zi * xy * x - xy * (xi * z + xz) + xx * (yz + yi * z) - x * (yi * xz + xi * yz) + 2 * xi * y * xz - 2 * zi * y * xx;
                        J_pw = A_star * E * (-1.0 * ddetA_dpw / detA / detA).transpose() + dAstarE_dpw / detA;
                        plane->plane_cov += J_pw * pv.cov_world * J_pw.transpose();
                    }
                }
                else
                {
                    ...
                    for (auto pv : points)
                    {
                        ...
                        dAstarE_dpw << zi * y * y - n * zi * yy + n * yi * yz - yz * y + yy * z - yi * y * z,
                            2 * xz * y - 2 * n * yi * xz + n * (xi * yz + zi * xy) - x * (yz + zi * y) + 2 * yi * x * z - z * (xy + xi * y),
                            xi * y * y - n * xi * yy + n * yi * xy - yi * x * y + yy * x - xy * y,
                            n * (yi * xz + zi * xy) - y * (xz + zi * x) + 2 * yz * x - 2 * n * xi * yz + 2 * xi * y * z - z * (yi * x + xy),
                            n * xi * xz - x * xz + zi * x * x - n * zi * xx + xx * z - xi * x * z,
                            n * xi * xy - xi * x * y + yi * x * x - n * yi * xx + xx * y - xy * x,
                            yy * (xz + zi * x) - y * (zi * xy + yi * xz) + 2 * xi * yz * y - yz * (xy + yi * x) + 2 * yi * z * xy - 2 * xi * z * yy,
                            2 * yi * xz * x - xz * (xi * y + xy) + xx * (yz + zi * y) - x * (zi * xy + xi * yz) + 2 * xi * z * xy - 2 * yi * z * xx,
        ...
        {
            if (temp_points_.size() > (size_t)update_size_threshold)
            {
                // init_plane(temp_points_, plane_ptr_);
                InitPlane(temp_points_, plane_ptr_, this);
                if (plane_ptr_->is_plane)
                {
                    if (temp_points_.size() > (size_t)max_points_size)
                    {
                            ...
                            if (iter != feat_map.end())
                            {
                                ...
                                if (neighbor_plane->is_plane)
                                {
                                    if (neighbor_plane->main_direction == now_plane->main_direction)
                                    {
                                        ...
                                        double m_distance = sqrt(abd_bias.transpose() * (neighbor_plane->plane_cov + now_plane->plane_cov).inverse() * abd_bias);
                                        if ((abd_bias[0] < 0.1 && abd_bias[1] < 0.1) || m_distance < 0.004)
                                        {
                                            ...
                                            double paraA                = neighbor_plane->plane_cov.norm() / (nowRealRootNode->plane_ptr_->plane_cov.norm() + neighbor_plane->plane_cov.norm());
                                            double paraB                = nowRealRootNode->plane_ptr_->plane_cov.norm() / (nowRealRootNode->plane_ptr_->plane_cov.norm() + neighbor_plane->plane_cov.norm());
                                            nowRealRootNode->plane_ptr_->n_vec =
                                                paraA * nowRealRootNode->plane_ptr_->n_vec + paraB * neighbor_plane->n_vec;
                                            nowRealRootNode->plane_ptr_->plane_cov =
                                                paraA * paraA * nowRealRootNode->plane_ptr_->plane_cov + paraB * paraB * neighbor_plane->plane_cov;
                                        }
                                    }
                                }
                            }
    ...
    /**
     * @brief Calculate the geometric bounds of the voxel map
     *
     * @param voxel_map Input voxel map to analyze
     * @param[out] min_bound Output minimum bounds (x,y,z)
     * @param[out] max_bound Output maximum bounds (x,y,z)
     * @param include_inactive Whether to include inactive (non-updatable) nodes only
     */
    void GetVoxelMapBounds(const std::unordered_map<VOXEL_LOC, UnionFindNode *> &voxel_map,
                           V3D &min_bound, V3D &max_bound,
                           bool include_inactive = true)
    {
        // Initialize bounds with extreme values
        min_bound = V3D(std::numeric_limits<double>::max(),
                        std::numeric_limits<double>::max(),
                        std::numeric_limits<double>::max());
        max_bound = V3D(std::numeric_limits<double>::lowest(),
                        std::numeric_limits<double>::lowest(),
                        std::numeric_limits<double>::lowest());

        for (const auto &kv : voxel_map)
        {
            // // Skip active nodes if requested
            // if (include_inactive && kv.second->update_enable_)
            // {
            //     continue;
            // }

            // Get voxel center coordinates
            const double x = kv.second->voxel_center_[0];
            const double y = kv.second->voxel_center_[1];
            const double z = kv.second->voxel_center_[2];
        ...
        if (min_bound.x() == std::numeric_limits<double>::max())
        {
            min_bound = V3D::Zero();
            max_bound = V3D::Zero();
        }
                             ...
                             bool &is_sucess, ptpl &single_ptpl)
    {
        if (currentNode->plane_ptr_->is_plane)
        {
            ...
            single_ptpl.point_cov      = pv.cov_world;
            // single_ptpl.point_cov = pv.cov_lidar;  //! voxelmap++ 不稳定的一个原因
            /*** Distance Covariance Propagation ***/
            double omg_norm_2 = single_ptpl.omega_norm * single_ptpl.omega_norm;
            switch (plane.main_direction)
            {
            case 0:
                single_ptpl.omega << plane.n_vec[0], plane.n_vec[1], 1.0;
                single_ptpl.dist = (pv.point_world.dot(single_ptpl.omega) + plane.n_vec[2]) / single_ptpl.omega_norm;
                J_abd << pv.point_world(0) - plane.n_vec[0] * single_ptpl.dist / omg_norm_2,
                    pv.point_world(1) - plane.n_vec[1] * single_ptpl.dist / omg_norm_2,
                    1.0;
                break;

            case 1:
                single_ptpl.omega << plane.n_vec[0], 1.0, plane.n_vec[1];
                single_ptpl.dist = (pv.point_world.dot(single_ptpl.omega) + plane.n_vec[2]) / single_ptpl.omega_norm;
                J_abd << pv.point_world(0) - plane.n_vec[0] * single_ptpl.dist / omg_norm_2,
                    pv.point_world(2) - plane.n_vec[1] * single_ptpl.dist / omg_norm_2,
                    1.0;
                break;

            case 2:
                single_ptpl.omega << 1.0, plane.n_vec[0], plane.n_vec[1];
                single_ptpl.dist = (pv.point_world.dot(single_ptpl.omega) + plane.n_vec[2]) / single_ptpl.omega_norm;
                J_abd << pv.point_world(1) - plane.n_vec[0] * single_ptpl.dist / omg_norm_2,
                    pv.point_world(2) - plane.n_vec[1] * single_ptpl.dist / omg_norm_2,
                    1.0;
                break;
            }
            ...
            const double sigma_l = J_abd * plane.plane_cov * J_abd.transpose() + (J_pw * pv.cov_world * J_pw.transpose()).value();

            // 3 Sigma Outlier Remove
            is_sucess = std::abs(single_ptpl.dist) < sigma_num * std::sqrt(sigma_l);
        }
    }
...
#pragma omp parallel for
    ...
    Eigen::Matrix3d calcLidarCov(V3D &pb, const float range_inc,
